...
export default function JiraShowcase({ grouped = false }) {
  const { t } = useTranslation();
  const features = t("jira.features", { returnObjects: true });
    ...
    <section id="jira" style={{ position: "relative", background: grouped ? "transparent" : DARK, color: "#fff", overflow: "hidden", scrollMarginTop: 72 }}>
      ...
      <div className="max-w-6xl mx-auto px-5 py-20 md:py-28 relative grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
        ...
        <Reveal delay={150} className="order-1 lg:order-2">
          <p style={{ ...mono, fontSize: 13, letterSpacing: ".12em", color: "#7FB0FF" }}>{t("jira.section.eyebrow")}</p>
          <h2 style={{ ...display, fontWeight: 700, fontSize: "clamp(26px,4vw,42px)", marginTop: 10, lineHeight: 1.15 }}>
            {t("jira.section.titleA")}{" "}
            <span style={gradientText}>{t("jira.section.titleB")}</span>
          </h2>
