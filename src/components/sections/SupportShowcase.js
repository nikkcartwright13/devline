...
import SupportMockup from "../ui/SupportMockup";
import Icon from "../ui/Icon";
import { upperLabel } from "../../lib/text";

const STACK = ["Jira Service Management", "PagerDuty", "Datadog", "Slack"];

export default function SupportShowcase() {
  const { t } = useTranslation();
  const features = t("support.features", { returnObjects: true });
  ...
  return (
    <section style={{ position: "relative", background: DARK, color: "#fff", overflow: "hidden" }}>
      ...
      <div className="max-w-6xl mx-auto px-5 py-20 md:py-24 relative grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
        {/* text */}
        <Reveal>
          <p style={{ ...mono, fontSize: 13, letterSpacing: ".12em", color: "#7FB0FF" }}>{t("services.items.support.eyebrow")}</p>
          ...
          <div style={{ marginTop: 28 }}>
            <p style={{ ...mono, fontSize: 11.5, letterSpacing: ".1em", color: "#93A5CE" }}>
              {upperLabel(t("support.builtWith"))}
            </p>
