...
import MetaMarketingMockup from "../ui/MetaMarketingMockup";
import Icon from "../ui/Icon";

const STACK = ["Meta Ads Manager", "Meta Pixel", "Instagram", "Facebook"];

export default function MetaMarketingShowcase() {
  const { t } = useTranslation();
  const features = t("metaMarketing.features", { returnObjects: true });
  ...
  return (
    <section style={{ position: "relative", background: DARK, color: "#fff", overflow: "hidden" }}>
      ...
      <Orb size={280} bottom="-110px" right="-60px" colors={["#F472B6", "transparent"]} dur={16} opacity={0.35} />

      <div className="max-w-6xl mx-auto px-5 py-20 md:py-24 relative grid lg:grid-cols-2 gap-14 items-center">
        {/* text */}
        <Reveal>
          <p style={{ ...mono, fontSize: 13, letterSpacing: ".12em", color: "#7FB0FF" }}>{t("services.items.metaMarketing.eyebrow")}</p>
          <h2 style={{ ...display, fontWeight: 700, fontSize: "clamp(26px,4vw,42px)", marginTop: 10, lineHeight: 1.15 }}>
            {t("services.items.metaMarketing.title")}
          </h2>
          <p style={{ color: "#B9C6E6", marginTop: 16, lineHeight: 1.8, maxWidth: 480 }}>
            {t("services.items.metaMarketing.text")}
          </p>
          ...
          <div style={{ marginTop: 28 }}>
            <p style={{ ...mono, fontSize: 11.5, letterSpacing: ".1em", color: "#93A5CE" }}>
              {t("metaMarketing.builtWith").toUpperCase()}
            </p>
