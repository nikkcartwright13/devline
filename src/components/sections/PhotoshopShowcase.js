...
import PhotoshopMockup from "../ui/PhotoshopMockup";
import Icon from "../ui/Icon";
import { upperLabel } from "../../lib/text";

const STACK = ["Adobe Photoshop", "Adobe Lightroom"];

export default function PhotoshopShowcase() {
  const { t } = useTranslation();
  const features = t("photoshop.features", { returnObjects: true });

  return (
    <section style={{ position: "relative", background: DARK, color: "#fff", overflow: "hidden" }}>
      <Orb size={320} top="-100px" left="-80px" colors={[T.violet, "transparent"]} dur={13} />
      <Orb size={280} bottom="-110px" right="-60px" colors={[T.blue, "transparent"]} dur={16} opacity={0.45} />

      <div className="max-w-6xl mx-auto px-5 py-20 md:py-24 relative grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
        {/* text */}
        <Reveal>
          <p style={{ ...mono, fontSize: 13, letterSpacing: ".12em", color: "#7FB0FF" }}>{t("services.items.photoshop.eyebrow")}</p>
          <h1 style={{ ...display, fontWeight: 700, fontSize: "clamp(26px,4vw,42px)", marginTop: 10, lineHeight: 1.15 }}>
            {t("services.items.photoshop.title")}
          </h1>
          <p style={{ color: "#B9C6E6", marginTop: 16, lineHeight: 1.8, maxWidth: 480 }}>
            {t("services.items.photoshop.text")}
          </p>
          ...
          <div style={{ marginTop: 28 }}>
            <p style={{ ...mono, fontSize: 11.5, letterSpacing: ".1em", color: "#93A5CE" }}>
              {upperLabel(t("photoshop.builtWith"))}
            </p>
