...
import SeoMockup from "../ui/SeoMockup";
import Icon from "../ui/Icon";

const STACK = ["Google Search Console", "Ahrefs", "Google Analytics 4"];

export default function SeoShowcase() {
  const { t } = useTranslation();
  const features = t("seo.features", { returnObjects: true });
  ...
  return (
    <section style={{ position: "relative", background: DARK, color: "#fff", overflow: "hidden" }}>
      ...
      <div className="max-w-6xl mx-auto px-5 py-20 md:py-24 relative grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
        {/* text */}
        <Reveal>
          <p style={{ ...mono, fontSize: 13, letterSpacing: ".12em", color: "#7FB0FF" }}>{t("services.items.seo.eyebrow")}</p>
